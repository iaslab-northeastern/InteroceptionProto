...
Pod::Spec.new do |s|
  ...
  s.source           = { :git => "git@github.com:biobeats/HANAudioEngine.git", :tag => '2.0.3' }
  s.social_media_url = 'http://www.biobeats.com'
  s.platform     = :ios, '7.0'
  s.requires_arc = true
  s.source_files = ['*.pd','engineabs/*.pd','roblib/*.{pd,wav}','appsounds/*/*.{wav,m4a}', 'rjbsd/*.{pd,txt}','fluid/*.{pd,txt}','HAN_engineabs/*.{pd}', 'packs/*/*.{wav,m4a}','packs/*/*/*.{wav,m4a}']
